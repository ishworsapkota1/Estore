...
exports.forgotPassword = async (req, res) => {
  ...
  const url = `${process.env.FRONTEND_URL}/forgotpassword/${token.token}`;
...
exports.login = async (req, res) => {
  ...
  let passwordCheck = await bcrypt.compare(password, users.password);
  if (!passwordCheck) {
    return res.status(400).json({ error: "Email and password do not match" });
  }
